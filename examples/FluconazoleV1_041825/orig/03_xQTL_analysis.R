...
#vcf.qs.file='/media/hoffman2/PUBLIC_SHARED/yeast/reference/1011/mega_filtered.qs'
vcf.qs.file='/media/hoffman2/PUBLIC_SHARED/yeast/reference/3000/VCFs_SNPs/Z_1011_filtered.qs'
...
vcf.qs=qs2::qs_read(vcf.qs.file) #'/data1/yeast/reference/pop_vcfs/1011/mega_filtered.qs')

#mega_filtered=qs::qread(vcf.qs.file)
...
for(n in 1:nrow(samples)) {
   print(n) 
    ...
    # countdf0 <- makeCountTablesGATK(count_dir, sname, p.names, vcf, gt, gmap)
    countdf <- makeCountTablesGATK2(count_dir, sname, p.names,  vcf.qs, gmap)
...
ggarrange(plots[[87]], plots[[82]], nrow=2)
